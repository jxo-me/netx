...
import (
	...
	"github.com/jxo-me/netx/x/internal/util/cache"
...
func (h *routerHandler) handleAssociate(ctx context.Context, conn net.Conn, network, host string, routerID relay.TunnelID, log logger.ILogger) (err error) {
...
func (h *routerHandler) handlePacket(ctx context.Context, data []byte, routerID relay.TunnelID, log logger.ILogger) error {
	var dstIP net.IP
...
func (h *routerHandler) getRoute(ctx context.Context, rid string, dst string) *router.Route {
	...
	if r := app.Runtime.RouterRegistry().Get(rid); r != nil {
