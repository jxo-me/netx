...
func NewTrafficLimiter(opts ...Option) traffic.ITrafficLimiter {
	var options options
...
func (l *trafficLimiter) In(ctx context.Context, key string, opts ...limiter.Option) traffic.ILimiter {
...
// For connection scope, the key should be client connection address.
func (l *trafficLimiter) Out(ctx context.Context, key string, opts ...limiter.Option) traffic.ILimiter {
	var options limiter.Options
	for _, opt := range opts {
		opt(&options)
	}
