...
func NewTrafficLimiter(opts ...Option) ITrafficLimiter {
	var options options
...
func (l *trafficLimiter) In(key string) Limiter {
	var lims []Limiter

	// service level limiter
	if lim, ok := l.inLimits.Get(GlobalLimitKey); ok && lim != nil {
		lims = append(lims, lim.(Limiter))
	}
...
// The key should be client connection address.
func (l *trafficLimiter) Out(key string) Limiter {
	var lims []Limiter
