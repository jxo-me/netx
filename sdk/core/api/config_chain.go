...
func createChain(ctx *gin.Context) {
	...
	if err := sdk.Runtime.ChainRegistry().Register(req.Data.Name, v); err != nil {
...
func updateChain(ctx *gin.Context) {
	...
	if !sdk.Runtime.ChainRegistry().IsRegistered(req.Chain) {
	...
	sdk.Runtime.ChainRegistry().Unregister(req.Chain)

	if err := sdk.Runtime.ChainRegistry().Register(req.Chain, v); err != nil {
		writeError(ctx, ErrDup)
		return
	}
