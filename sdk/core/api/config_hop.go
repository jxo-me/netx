...
func createHop(ctx *gin.Context) {
	...
	if err := sdk.Runtime.HopRegistry().Register(req.Data.Name, v); err != nil {
...
func updateHop(ctx *gin.Context) {
	...
	if !sdk.Runtime.HopRegistry().IsRegistered(req.Hop) {
	...
	sdk.Runtime.HopRegistry().Unregister(req.Hop)

	if err := sdk.Runtime.HopRegistry().Register(req.Hop, v); err != nil {
		writeError(ctx, ErrDup)
		return
	}
