...
type promMetrics struct {
	...
	counters   map[MetricName]*prometheus.CounterVec
	histograms map[MetricName]*prometheus.HistogramVec
}
...
func (m *promMetrics) Gauge(name MetricName, labels Labels) IGauge {
	v, ok := m.gauges[name]
	if !ok {
		return nil
	}
...
func (m *promMetrics) Counter(name MetricName, labels Labels) ICounter {
...
func (m *promMetrics) Observer(name MetricName, labels Labels) IObserver {
