...
func createConnLimiter(ctx *gin.Context) {
	...
	v := sdk.Runtime.ParseConnLimiter(&req.Data)

	if err := sdk.Runtime.ConnLimiterRegistry().Register(req.Data.Name, v); err != nil {
...
func updateConnLimiter(ctx *gin.Context) {
	...
	if !sdk.Runtime.ConnLimiterRegistry().IsRegistered(req.Limiter) {
	...
	sdk.Runtime.ConnLimiterRegistry().Unregister(req.Limiter)

	if err := sdk.Runtime.ConnLimiterRegistry().Register(req.Limiter, v); err != nil {
		writeError(ctx, ErrDup)
		return
	}
