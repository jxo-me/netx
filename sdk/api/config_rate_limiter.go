...
func createRateLimiter(ctx *gin.Context) {
	...
	v := sdk.Runtime.ParseRateLimiter(&req.Data)

	if err := sdk.Runtime.RateLimiterRegistry().Register(req.Data.Name, v); err != nil {
...
func updateRateLimiter(ctx *gin.Context) {
	...
	if !sdk.Runtime.RateLimiterRegistry().IsRegistered(req.Limiter) {
	...
	sdk.Runtime.RateLimiterRegistry().Unregister(req.Limiter)

	if err := sdk.Runtime.RateLimiterRegistry().Register(req.Limiter, v); err != nil {
		writeError(ctx, ErrDup)
		return
	}
