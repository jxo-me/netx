...
func createIngress(ctx *gin.Context) {
	...
	v := sdk.Runtime.ParseIngress(&req.Data)

	if err := sdk.Runtime.IngressRegistry().Register(req.Data.Name, v); err != nil {
...
func updateIngress(ctx *gin.Context) {
	...
	if !sdk.Runtime.IngressRegistry().IsRegistered(req.Ingress) {
	...
	sdk.Runtime.IngressRegistry().Unregister(req.Ingress)

	if err := sdk.Runtime.IngressRegistry().Register(req.Ingress, v); err != nil {
		writeError(ctx, ErrDup)
		return
	}
